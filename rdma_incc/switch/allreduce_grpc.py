def NameSpace(): # run all the code in a function, because this file is executed through exec() which will cause some namespace problems
    ...
    class INCServicerImpl(allreduce_pb2_grpc.INCServicer):
        ...
        def CreateGroup(self, request, context):
            ...
            for i in range(group_size):
                self.debug_exec(f"self.program.pipe.Ingress.metadata_table.add_with_get_allreduce_metadata\
    (sip={request.Member[i].IP:#x}, dip={switch_IP}, dqpn={request.Member[i].QPN}, group_id={groupid}, src_rank={i}, \
    bitmap={1<<i:#x}, bitmap_mask={(1<<group_size)-1:#x}, agg_addr={agg_addr:#x}, \
    ...
    (group_id={groupid}, src_rank={i}, valid={1}, sip={switch_IP}, dip={request.Member[i].IP:#x}, dqpn={request.Member[i].QPN:#x}, rkey={request.Member[i].RKEY:#x})")
                self.debug_exec(f"self.program.pipe.Egress.restore_table.add_with_restore_fields\
    (group_id={groupid}, src_rank={i}, valid={0}, sip={switch_IP}, dip={request.Member[i].IP:#x}, dqpn={request.Member[i].QPN:#x})")
                undo_funcs.append(f"self.program.pipe.Egress.restore_table.delete\
    (group_id={groupid}, src_rank={i}, valid={1})")
                undo_funcs.append(f"self.program.pipe.Egress.restore_table.delete\
    (group_id={groupid}, src_rank={i}, valid={0})")
