...
parser EgressParser(packet_in packet,
    ...
    state start {
        ...
        transition select(hdr.bridge.pass_type) {
            ...
            REDUCE_BACKWARD_PASS : parse_conn;
...
//     }
    ...
    apply { 
        ...
        if(hdr.bridge.pass_type == REDUCE_BACKWARD_PASS) {
            hdr.conn.dst_rank = eg_intr_md.egress_rid;
        }// else hdr.conn.dst_rank is the root rank
...
// #endif
            ...
            // ack_table.apply();
            hdr.recir.setInvalid();// only for ALLREDUCE_SECOND_PASS
            hdr.conn.setInvalid();// both for ALLREDUCE_SECOND_PASS and REDUCE_BACKWARD_PASS
