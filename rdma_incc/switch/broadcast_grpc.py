def NameSpace(): # run all the code in a function, because this file is executed through exec() which will cause some namespace problems
    ...
    class INCServicerImpl(allreduce_pb2_grpc.INCServicer):
        ...
        def CreateGroup(self, request, context):
            ...
            if len(self.free_group) < 1 or len(self.free_node) < group_size:
                print("No resource")
            ...
            for i in range(group_size):
                if i == request.RootRank:
                    self.debug_exec(f"self.program.pipe.Ingress.metadata_table.add_with_get_forward_metadata\
        (sip={request.Member[i].IP:#x}, dip={switch_IP}, dqpn={request.Member[i].QPN}, group_id={groupid}, src_rank={i}, root_rank={request.RootRank})")
                else:
                    self.debug_exec(f"self.program.pipe.Ingress.metadata_table.add_with_get_backward_metadata\
        (sip={request.Member[i].IP:#x}, dip={switch_IP}, dqpn={request.Member[i].QPN}, group_id={groupid}, src_rank={i}, root_rank={request.RootRank}, \
        bitmap={1<<i:#x}, bitmap_mask={((1<<group_size)-1)^(1<<request.RootRank):#x})")
    ...
    (group_id={groupid}, dst_rank={i}, valid={0})")
                ...
                # Since we use XID, we do not skip the root
            ...
            # Using XID to exclude the root rank in P4
            self.debug_exec(f"bfrt.pre.mgid.add({groupid}, {node}, {[True]*group_size}, {list(range(group_size))})")
            undo_funcs.append(f"bfrt.pre.mgid.delete({groupid})")

            self.debug_exec(f"self.program.pipe.Egress.broadcast_ack_egress.reg_bitmap.mod({groupid}, 0)")
            self.debug_exec(f"self.program.pipe.Egress.broadcast_ack_egress.reg_prev_psn.mod({groupid}, 0xffffff00)")
            self.debug_exec(f"self.program.pipe.Egress.broadcast_ack_egress.reg_prev_msn.mod({groupid}, 0)")
